...
exports.login = (req, res, callback) => {
    ...
    pool.query(query.getUserByEmail, [email]).then(result => {
        if (result.rowCount == 0) return callback(constants.userNotFound, 401);
        if (hashPass(password) != result.rows[0].password) return callback(constants.wrongPass, 401);

        const user = result.rows[0]
        const token_data = { id: user.id, email: email}

        let toot = generateUserToken(token_data, callback)
        console.log(toot)
...
exports.add = (req, res, callback) => {
    ...
    pool.query(query.getUserByEmail, [email]).then(result => {
        if (result.rowCount > 0) return callback(constants.existentEmail, 404);
